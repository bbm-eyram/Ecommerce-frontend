...
export default function Cart() {
  ...
  async function stripeCheckout() {
    const response = await axios.post('/api/checkout', {
      email: session.user.email, name: session.user.name, address, contact, state, zip, city, cartProducts
    });
                        ...
                        <li className="flex items-center gap-4 justify-between">
                          ...
                          <div>
                            ...
                            <dl className="mt-0.5 space-y-px text-[10px] text-text">
                              <p>GHc {cartProducts.filter(id => id === product._id).length * product.price}</p>
                            </dl>
                          </div>
                    ...
                    <div className=" max-w-md space-y-4">
                      <dl className="space-y-0.5 text-md text-gray-700">
                        ...
                        <div className="flex justify-between">
                          <dt>Subtotal</dt>
                          <dd>GHc {formatPrice(total)}</dd>
                        </div>

                        <strike className="flex justify-between">
                          <dt>VAT</dt>
                          <dd>GHc {formatPrice(total / 1000)}</dd>
                        </strike>
                      ...
                      <div className="flex justify-end" style={{ marginBottom: '15px' }}>
                        <Link
                          class="group flex items-center justify-between gap-4 rounded-lg border border-current px-4 py-2 text-blue-600 transition-colors hover:bg-blue-600 focus:outline-none focus:ring active:bg-blue-500"
                          href="/products"
                        >
                          ...
                          <span
                            class="shrink-0 rounded-full border border-blue-600 bg-white p-2 group-active:border-blue-500"
                          >
                            <svg
        ...
        {!products.length ? (
          ...
          <div className="md:1/3 mt-16 md:mt-6">
            <header className="text-start flex flex-col w-full">
              ...
              <p className="mt-2 text-text text-lg">We use your account details for shipping and delivery.</p>
            </header>
              ...
              <div class="space-y-5">
                <div class="grid grid-cols-12 gap-5">
                  ...
                  <div class="col-span-12">
                    <label class="mb-1 block text-sm font-medium text-text">Contact</label>
                    <input type="text" name="address" class="block w-full rounded-md p-3 border border-gray-300 shadow-sm focus:border-primary-400 focus:ring focus:ring-primary-200 focus:ring-opacity-50 disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-500" placeholder="+233 555 555 555"
                      value={contact}
